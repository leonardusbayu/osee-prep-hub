...
class _AiGraderPageState extends ConsumerState<AiGraderPage> {
  ...
  Future<void> _submit() async {
    ...
    setState(() { _isSubmitting = true; _status = 'submitting'; });
    try {
      final dio = ApiClient.create();
      final response = await dio.post('/ai/grade-writing', data: {
        'essay': _essayController.text,
        'rubric': _rubric,
        'examType': _examType,
        'level': _level,
      });
      final queueId = response.data['queue_id'] as String?;
            ...
            children: [
              ...
              TextFormField(
                ...
                decoration: const InputDecoration(
                  labelText: 'Essay to grade (250+ words recommended)',
                  alignLabelWithHint: true,
                ),
                maxLines: 12,
                minLines: 8,
                validator: (v) => (v == null || v.trim().length < 50) ? 'Essay too short' : null,
              ),
                ...
                children: [
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      ...
                      items: const [
                        DropdownMenuItem(value: 'ielts_task1', child: Text('IELTS Task 1')),
                        DropdownMenuItem(value: 'ielts_task2', child: Text('IELTS Task 2')),
                      ...
                      onChanged: (v) => setState(() => _rubric = v ?? 'ielts_task2'),
                    ),
                  ),
              ...
              DropdownButtonFormField<String>(
                value: _level,
                decoration: const InputDecoration(labelText: 'CEFR Level (target)'),
                items: const [
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  ...
                  color: isComplete ? Colors.green : (status == 'error' ? Colors.red : Colors.orange),
                ),
            ...
            if (isComplete) ...[
              const SizedBox(height: 16),
              Text(_result!['feedback'] as String? ?? '', style: Theme.of(context).textTheme.bodyMedium),
              const SizedBox(height: 16),
              Text('Criteria Breakdown', style: Theme.of(context).textTheme.titleSmall),
              ...((_result!['criteria_scores'] as List?)?.map((c) => ListTile(
              ...
              ...((_result!['improvements'] as List?)?.map((i) => Padding(
